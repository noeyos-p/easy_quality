...
2단계: 인용된 조항이 컨텍스트에 실제로 존재하는지 검증
   - 답변에 "(EQ-SOP-00001 > 5.1.3)"이라고 인용했다면
   - 컨텍스트에서 EQ-SOP-00001의 5.1.3 조항을 찾으세요
   - 없다면 할루시네이션입니다
...
- 인용된 조항이 컨텍스트에 없으면 즉시 대폭 감점하세요

JSON 형식으로 답변하세요:
{{"score": 1-5, "reasoning": "평가 근거를 한국어로 3-4문장으로 설명. 컨텍스트에 없는 조항이 있다면 구체적으로 지적"}}"""
...
[평가 절차 - 반드시 순서대로 수행]
1단계: 답변에서 (문서명 > 조항) 형식의 인용을 모두 찾으세요
2단계: 각 인용에 대해 검증:
   - 컨텍스트에서 해당 문서의 해당 조항 내용을 찾으세요
   - 답변의 해당 문장이 실제로 그 조항의 내용과 일치하는지 확인하세요
   - 조항 번호가 잘못 매핑되었다면 즉시 감점하세요
3단계: 전체 답변의 근거성을 종합 평가하세요

[평가 기준 - 매우 엄격하게 적용]
5점:
  - 모든 문장에 (문서명 > 조항) 인용이 있음
  - 모든 인용이 컨텍스트의 실제 조항 내용과 정확히 일치함
  - 조항 번호 오류 0개

4점:
  - 대부분 인용 있으나 1-2개 문장에서 누락
  - 인용된 조항이 대부분 정확하나 1개 정도 불명확
  - 조항 번호 오류 0-1개

3점:
  - 절반 정도만 인용 있음
  - 또는 인용은 있으나 조항 번호가 실제 내용과 맞지 않는 경우 2-3개
  - "일반적으로", "보통" 같은 근거 없는 표현 있음

2점:
  - 인용이 매우 적음 (30% 미만)
  - 또는 인용된 조항 번호의 50% 이상이 실제 내용과 불일치
  - 근거 없는 주장 다수
...
**핵심 검증 사항**:
- 답변: "주요 특징은... (EQ-SOP-00001 > 5.2.2.2.2)"
- 검증: 컨텍스트에서 EQ-SOP-00001의 5.2.2.2.2 조항을 찾아 실제로 "주요 특징"에 대한 내용인지 확인
- 없으면 할루시네이션이므로 점수를 대폭 감점

JSON 형식으로 답변하세요:
{{"score": 1-5, "reasoning": "평가 근거를 한국어로 3-4문장으로 설명. 조항 번호 오류가 있다면 구체적으로 지적"}}"""
...
2단계: 인용된 조항 번호가 실제 내용과 일치하는지 검증
   - 답변: "특징은... (EQ-SOP-00001 > 5.2.2.2.2)"
   - 검증: 컨텍스트에서 5.2.2.2.2 조항을 찾아 실제로 "특징"에 대한 내용인지 확인
...
**중요**: 조항 번호가 틀린 것은 단순 실수가 아닌 심각한 할루시네이션입니다. 즉시 대폭 감점하세요.

JSON 형식으로 답변하세요:
{{"score": 1-5, "reasoning": "평가 근거를 한국어로 3-4문장으로 설명. 조항 번호 오류가 있다면 구체적으로 지적하고 몇 개나 틀렸는지 명시"}}"""
