    ...
    try:
        ...
        # [CASE 2] 버전 비교
            ...
            try:
                ...
                # 보고서용 데이터 포맷팅 (ADDED, DELETED, MODIFIED)
                comp_lines = []
                
                # 1. 추가/삭제된 조항 먼저 수집
                added_deleted = [d for d in diffs if d.get('change_type') in ['ADDED', 'DELETED']]
                for item in added_deleted:
                    ctype = item.get('change_type')
                    clause_id = item.get('clause') or "N/A"
                    content = (item.get('v2_content') if ctype == 'ADDED' else item.get('v1_content')) or ""
                    comp_lines.append(f"### [상태: {ctype}] 조항 {clause_id}\n내용: {content.strip()}\n")
                ...
                for item in diffs:
                    if item.get('change_type') == 'MODIFIED':
                        ...
                        if v1_txt.replace(" ", "").replace("\n", "") != v2_txt.replace(" ", "").replace("\n", ""):
                            comp_lines.append(f"### [상태: MODIFIED] 조항 {clause_id}\n[변경 전 원문]\n{v1_txt}\n\n[변경 후 원문]\n{v2_txt}\n")

                comp_data = "\n".join(comp_lines)
                if not comp_data:
                    comp_data = "데이터 비교 결과, 추가, 삭제되거나 실제 텍스트 내용이 변경된 조항이 발견되지 않았습니다."
...
8. **가독성 및 레이아웃 (필수)**:
    ...
    - **색상 적용 (필수)**: 
        ...
        - '변경 후'이라는 3글자는 반드시 `<span style="color: #2db7f5">변경 후</span>`와 같이 HTML 태그를 사용하여 초록색(또는 청록색)으로 표기하세요.
